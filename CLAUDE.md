...
| `guard/` | `heresy-guard`, a Rust `PreToolUse` hook denying Bash and apidoc heresies. Has its own `CLAUDE.md` |
...
* Each guard catalogue and its rite are one rite in two forms — executable and written:
  `guard/src/catalogue.rs` ↔ `references/shell.md`, `guard/src/apidoc.rs` ↔ the apidoc catalogue
  in `references/code-style.md`. **Change one, change the other.** A denial quotes the written
  directive back at the offender, so drift makes the guard lie.
